...
- [ ] **Gate AD logins on the NFS `/home` mount (impermanence data-loss window).** With impermanence + the `nofail` `/home` mount, if fabricant (NFS) is down at boot `/home` is left unmounted; an SSSD-cached AD login then triggers `pam_mkhomedir` to create an ephemeral home on the rolled-back root, which is wiped on the next reboot. fabricant-down-at-boot is already a major incident (it also hosts the AD DC), but to close the silent-data-loss window, gate AD logins on `home.mount` being active (e.g. order sshd/the login path after it, or prevent `mkhomedir` when `/home` isn't a mountpoint). Needs its own design + test (don't lock out break-glass `krg-admin`, whose home is `/var/lib/<account>`).
