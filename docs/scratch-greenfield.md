...
| **ARC cap (64 GiB) + earlyoom** | Mixed GPU/CPU/FPGA jobs share one box and one finite cache; the cap stops ARC starving a RAM-hungry job, earlyoom handles real pressure gracefully. **64 GiB is a starting floor — tune to installed RAM** (≈25%, minus L2ARC header overhead). |
